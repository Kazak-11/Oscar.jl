...
function _lift_canonical_ordering(i, j, w, can_order) 
    can_i = find(can_order .== (1+(w-1)*(i-1)))[0] # returnes an index of min Si = (i,0) in the canonical ordering 
...
function canonical_form(L::ZZLat)
    ...
    elseif n>=5 #maybe it will be removed or this condition made higher due to performance
        char_vectors_set = _vor_vectors_set()
    end 
    T1_graph = _get_edge_labeled_graph(char_vectors_set, gram) # transform from the adjenctcy matrix A to the edge-vertex weighted graph Ga, then to the edge weighted graph T1(Ga)
    T2_graph =  _edge_label_to_vertex_label(T1_graph, label = :weight) # transform from T1(Ga) to the vertex weighted graph T2(T1(Ga))
    p = length(char_vectors_set)
    can_order = _canonical_perm(T2_graph, label = :weight) # get the canonical ordering of T2(T1(Ga))
